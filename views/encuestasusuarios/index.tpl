			...
			{foreach from=$encuestasUsuarios item=eu}
				<tr>
					...
					<td>
						...
						<a href="{$_layoutParams.root}encuestasusuarios/delete/{$eu.usuario_id}" class="btn btn-link btn-sm" onclick="return eliminar('{$_layoutParams.root}encuestasusuarios/delete/{$eu.usuario_id}', 'la asociación {$eu.encuesta} - {$eu.usuario}')">Eliminar</a>
					</td>
				</tr>
			{/foreach}		
		</table>
