...
class agendamientoContactoModel extends Model
{
	...
	public function getAgendamientosContactos(){
		$age = $this->_db->query("SELECT DISTINCT ag.id, ag.contacto_id, ag.fecha_agenda, ag.hora_agenda, ag.created_at as creado, ag.status, u.nombre as ejecutivo, c.nombre as contacto, c.modified_at as modificado, ell.nombre as llamada FROM agendamientos_contactos ag INNER JOIN usuarios u ON ag.ejecutivo_id = u.id INNER JOIN contactos c ON ag.contacto_id = c.id INNER JOIN estado_llamadas ell ON c.estado_llamada = ell.id ORDER BY ag.fecha_agenda");
	...
	public function getAgendamientoContactoId($id){
		...
		$age = $this->_db->prepare("SELECT DISTINCT ag.id, ag.ejecutivo_id, ag.contacto_id, ag.fecha_agenda, ag.hora_agenda, ag.created_at as creado, u.nombre as ejecutivo, c.nombre as contacto, c.modified_at as modificado, ag.status, ell.nombre as llamada FROM agendamientos_contactos ag INNER JOIN usuarios u ON ag.ejecutivo_id = u.id INNER JOIN contactos c ON ag.contacto_id = c.id INNER JOIN estado_llamadas ell ON c.estado_llamada = ell.id WHERE ag.id = ?");
		$age->bindParam(1, $id);
		$age->execute();
	...
	public function getAgendamientoUsuario($usuario){
		...
		$age = $this->_db->prepare("SELECT DISTINCT ag.id, ag.contacto_id, ag.fecha_agenda, ag.hora_agenda, ag.created_at as creado, u.nombre as ejecutivo, c.nombre as contacto, c.modified_at as modificado, ell.nombre as llamada FROM agendamientos_contactos ag INNER JOIN usuarios u ON ag.ejecutivo_id = u.id INNER JOIN contactos c ON ag.contacto_id = c.id INNER JOIN estado_llamadas ell ON c.estado_llamada = ell.id WHERE ag.ejecutivo_id = ? ORDER BY ag.fecha_agenda");
